...
// ── Inline status banner shown right after add action ─────────────────────────
...
function PosterManager() {
  ...
  const uploadMutation = useMutation({
    ...
    // Optimistic: show preview immediately in list
  ...
  const reorderMutation = useMutation({
    mutationFn: async (updates: any) => {
      const res = await fetch(`${API_URL}/posters/reorder/`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(updates),
      });
  ...
  const syncMutation = useMutation({
    mutationFn: async () => {
      const cloudName = "de7whmpgp";
      const tag = "Poster";
      const cloudRes = await fetch(
        `https://res.cloudinary.com/${cloudName}/image/list/${tag}.json?v=${Date.now()}`
      );
      if (!cloudRes.ok) throw new Error("Could not fetch from Cloudinary.");
      const data = await cloudRes.json();
      const resources = data.resources || [];
      if (resources.length === 0) throw new Error("No posters found with tag 'Poster' in Cloudinary.");
      const itemsToSync = resources.map((r: any) => ({
        public_id: r.public_id,
        secure_url: `https://res.cloudinary.com/${cloudName}/image/upload/v${r.version}/${r.public_id}.${r.format}`,
      }));
      const res = await fetch(`${API_URL}/posters/bulk-sync/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    onSuccess: (data) => {
      ...
      setBanner({ status: "success", message: `✓ ${data.message}` });
      toast.success(data.message);
      setTimeout(() => setBanner(null), 4000);
    },
    onError: (err: any) => {
      setBanner({ status: "error", message: `✗ Sync failed: ${err.message}` });
  ...
  const handleUpload = () => {
    if (!file) {
      setBanner({ status: "error", message: "✗ Please select a poster image first." });
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-4">
        ...
        <button
          ...
          className="flex-1 flex h-12 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 font-medium text-white transition hover:bg-white/10 disabled:opacity-50"
        >
          {syncMutation.isPending ? <Loader2 className="animate-spin h-5 w-5" /> : <RefreshCcw className="h-5 w-5" />}
      ...
      {/* POSTERS LIST */}
      <div className="space-y-3">
        <h3 className="text-xs uppercase tracking-widest text-cinema-muted mb-4">
                  ...
                  <>
                    <div className="flex items-center rounded bg-black/30 p-0.5">
                      <button
                        onClick={() => moveItem(idx, "up")}
                        disabled={idx === 0}
                        className="p-1 text-cinema-muted hover:text-white disabled:opacity-20"
                      >
                        <ArrowUp className="h-4 w-4" />
                      </button>
                      <button
